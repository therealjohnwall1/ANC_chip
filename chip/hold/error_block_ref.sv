...
(
  ...
  // eps widened into the accumulator fraction (update.py:107). Never zero,
  // so the divider needs no divide-by-zero guard.
  localparam accum_t EPS_ACC = accum_t'(EPS_RAW) <<< (ACC_FRAC - EPS_FRAC);

  // dividend is constant: lr pre-shifted so the quotient lands in step_t
  accum_t div_num;
  ...
  // per-tap update: exact products, single rounding at the Q1.15 write.
  // |delta_raw| < 2^61 and |sum_raw| < 2^62, so 64-bit math never wraps.
  ...
  // round half AWAY FROM ZERO: floor(x + 1/2) is only correct for x >= 0,
  // so round the magnitude and restore the sign
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      case (state)
        ...
        // E[n] = sum x_f^2: same tap walk as the update, multiplier fed
        // with the window on both operands
        S_ENERGY: begin
