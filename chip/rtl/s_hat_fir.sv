...
(
  ...
  typedef enum logic [1:0] { S_IDLE, S_CAP, S_FIR, S_ROUND } state_t;
  state_t state;
    ...
    if (!rst_n) begin
      ...
      case (state)
        ...
        S_FIR: begin
          ...
          if (fir_cnt == S_TAP_CNT_W'(S_TAP_LEN - 1))
            state <= S_ROUND;
          else
        ...
        S_ROUND: begin
          for (int k = XF_LINE_DEPTH - 1; k >= 1; k--)
            xf_line[k] <= xf_line[k-1];
          xf_line[0]  <= xf_out;
          x_f_ready   <= 1'b1;
          x_f_valid   <= 1'b1;
          state       <= S_IDLE;
