...
module anchor_top_tb;
  ...
  task automatic run_sample(input int xi, input int ei, input int idx);
    ...
    if (y_n !== y_exp[idx]) $fatal(1, "y[%0d] = %h, expected %h", idx, y_n, y_exp[idx]);
  ...
  initial begin
    ...
    for (int j = 0; j < ANC_N; j++) begin
      ...
      if ((j & 511) == 511)
        $display(
            "  sample %0d/%0d  y_n=%0d  e_q11=%0d",
            j + 1,
            ANC_N,
            y_n,
            $signed(
                {~err_data[11], err_data[10:0]}
            )
        );
    end
    ...
    for (int k = 0; k < TAP_LEN; k++) begin
      ...
      @(negedge clk);
      if (w_rd_data !== w_exp[k]) $fatal(1, "w[%0d] = %h, expected %h", k, w_rd_data, w_exp[k]);
    end

    $display("T=%0t: anchor_top_tb done -- %0d samples, y bit-exact vs golden, final w[] match.",
             $time, ANC_N);
    $finish;
  end
