...
module tt_um_anchor_tb;
  ...
  task automatic wait_flag(input int bit_idx);
    ...
    forever begin
      ...
      guard++;
      if (guard > 1000)
        $fatal(1, "wait_flag: timeout on STATUS bit %0d", bit_idx);
    end
  endtask
  ...
  task automatic run_sample(input int xi, input int ei, input int idx);
    ...
    y_rd = {y3, y2, y1, y0};
    if (y_rd !== y_exp[idx])
      $fatal(1, "y[%0d] = %h, expected %h", idx, y_rd, y_exp[idx]);
  ...
  initial begin
    ...
    begin
      logic [15:0] s;
      mmio_read(REG_STATUS, s);
      if (s[SB_XF_VALID] !== 1'b1)
        $fatal(1, "x_f_valid never went high after priming");
    end
    ...
    for (int j = 0; j < ANC_N; j++) begin
      ...
      if ((j & 511) == 511)
        $display("  sample %0d/%0d", j + 1, ANC_N);
    end
    ...
    for (int k = 0; k < TAP_LEN; k++) begin
      ...
      if (w !== w_exp[k])
        $fatal(1, "w[%0d] = %h, expected %h", k, w, w_exp[k]);
    end

    $display("T=%0t: tt_um_anchor_tb done -- %0d samples, y bit-exact vs golden, final w[] match.", $time, ANC_N);
    $finish;
  end
